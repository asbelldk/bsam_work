...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_Model1->Initialize(m_D3D->GetDevice(), L"../Engine/data/seafloor.dds", "../Engine/data/square.txt");
	if(!result)
	{
	...
	result = m_Model2->Initialize(m_D3D->GetDevice(), L"../Engine/data/seafloor2.dds", "../Engine/data/square.txt");
	if(!result)
	{
	...
	m_TextureShader = new TransparentShaderClass;
	if(!m_TextureShader)
	{
		return false;
	}
...
bool GraphicsClass::Render()
{
	...
	{
		if (blendAmount2 < 1.0f)
			blendAmount2 += 0.01f;
	}

	if (::GetAsyncKeyState(VK_LEFT) & 0x8000)
	{
		if (blendAmount2 > 0.0f)
			blendAmount2 -= 0.01f;
	}
	...
	result = m_TextureShader->Render(m_D3D->GetDeviceContext(), m_Model1->GetIndexCount(), worldMatrix, viewMatrix,
									 projectionMatrix, m_Model1->GetTexture(), blendAmount2);
	if(!result)
	{
		return false;
	}
